...
function init(env, ctx, server) {
  ...
  function start() {
    io = require("socket.io")({
      ...
      pingInterval: 35000, // Set the ping interval to 35 seconds, +10 seconds from inital.
      pingTimeout: 70000, // Recommended new value
    });
  ...
  function listeners() {
    io.sockets.on("connection", function onConnection(socket) {
      ...
      socket.on("dbUpdate", function dbUpdate(data, callback) {
        ...
        ctx.store.collection(collection).update({ _id: id }, { $set: data.data }, function (err, results) {
          if (!err) {
            ctx.store.collection(collection).findOne({ _id: id }, function (err, results) {
      ...
      socket.on("dbUpdateUnset", function dbUpdateUnset(data, callback) {
        ...
        ctx.store.collection(collection).update({ _id: objId }, { $unset: data.data }, function (err, results) {
          if (!err) {
            ctx.store.collection(collection).findOne({ _id: objId }, function (err, results) {
              console.log("Got results", results);
              if (!err && results !== null) {
                ctx.bus.emit("data-update", {
                  type: data.collection,
      ...
      socket.on("dbRemove", function dbRemove(data, callback) {
        ...
        var objId = new ObjectID(data._id);
        ctx.store.collection(collection).remove({ _id: objId }, function (err, stat) {
          if (!err) {
            ctx.bus.emit("data-update", {
              type: data.collection,
              op: "remove",
