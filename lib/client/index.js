  ...
  $.ajax({
    ...
    .fail(function fail(jqXHR) {
      ...
      if (client.settingsFailed) {
        ...
        var lang =
          Storages.localStorage.get("language") || (navigator.language || navigator.userLanguage).toLowerCase();
...
client.loadLanguage = function loadLanguage(serverSettings, callback) {
  ...
  $.ajax({
    method: "GET",
    url: "https://assets.nightscout.host/translations/" + filename + "?v3",
  })
...
client.load = function load(serverSettings, callback) {
  ...
  client.tooltip = d3.select("body").append("div").attr("class", "tooltip").style("opacity", 0);
  ...
  client.afterAuth = function afterAuth(isAuthenticated) {
    ...
    $("#treatmentDrawerToggle").toggle(
      treatmentCreateAllowed && client.settings.showPlugins.indexOf("careportal") > -1
    );
    ...
    editButton.click(function editModeClick(event) {
      ...
      if (client.editMode) {
        ...
        chart.focus.selectAll(".draggable-treatment").style("cursor", "default").on("mousedown.drag", null);
  ...
  function brushed() {
    ...
    function updateHeader() {
      if (inRetroMode()) {
        ...
        $("#currentTime").text(formatTime(nowDate, true)).css("text-decoration", "line-through");
      } else {
  ...
  alarmSocket.on("urgent_alarm", function (notify) {
    ...
    var enabled =
      (isAlarmForHigh() && client.settings.alarmUrgentHigh) || (isAlarmForLow() && client.settings.alarmUrgentLow);
    if (enabled) {
  ...
  function prepareEntries() {
    ...
    if (client.ddata.cal && client.rawbg.isEnabled(sbx)) {
      temp1 = client.ddata.sgvs
        .map(function (entry) {
          var rawbgValue = client.rawbg.showRawBGs(entry.mgdl, entry.noise, client.ddata.cal, sbx)
